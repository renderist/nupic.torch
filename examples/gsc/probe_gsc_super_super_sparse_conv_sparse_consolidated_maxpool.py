...
def conv_unroll(tnsor, dim1, dim0):
    siz = tnsor.size()
    assert siz[0] == 1
    nsiz0 = siz[-1]
    nsiz1 = siz[-2]
    nsteps0 = nsiz0 - dim0 + 1;
    nsteps1 = nsiz1 - dim1 + 1;
    nl = siz[-3]*dim1*dim0
    ret = torch.tensor([])
    flag = 0
    for j in range(nsteps1):
        for i in range(nsteps0):
            tmp = tnsor[0,:,j:(j+dim1), i:(i+dim0)]
            vw = tmp.reshape((1, nl))
            if flag:
                ret = torch.cat((ret,vw),axis=0)
            else:
                ret = vw
                flag = 1
    return ret
...
def print_sparse_conv_state(self, input, output):
    global sparse_conv_weights_var, sparse_conv_mults_var, sparse_conv_weights, sparse_conv_biases, sparse_conv_instances, sparse_conv_verify
    global first_time
    if first_time:
        print_layer(self, input, output)
        sparse_conv_weights = self.module.weight.data
        nmaps = sparse_conv_weights.size()[0]
        dim0 = sparse_conv_weights.size()[-1]
        dim1 = sparse_conv_weights.size()[-2]
        # Collapse to 2D matrix, columns = filter weights/map, rows = maps
        sparse_conv_weights = sparse_conv_weights.view(nmaps, int(sparse_conv_weights.numel()/nmaps))
        sparse_conv_biases = self.module.bias.data
        sparse_conv_weights_var = (sparse_conv_weights != 0).int()
        sparse_conv_mults_var = 0 # count_mults(self.module.weight.data, input[0])
        sparse_conv_instances = conv_unroll(input[0], dim1, dim0)
        sparse_conv_verify = output.data.reshape(1, output.data.numel())
    else:
        siz2 = self.module.weight.data.size()
        dim0 = siz2[-1]
        dim1 = siz2[-2]
        #tmp = count_mults(self.module.weight.data, input[0])
        sparse_conv_mults_var = 0 # torch.cat((sparse_conv_mults_var, tmp), axis=1)
        sparse_conv_instances = torch.cat((sparse_conv_instances,conv_unroll(input[0], dim1, dim0)), axis=0)
        sparse_conv_verify = torch.cat((sparse_conv_verify, output.data.reshape(1, output.data.numel())), axis=0)
...
if __name__ == "__main__":
    ...
    model = load_le_sparse_net("/Users/khunter/nta/nupic.torch/examples/gsc/model_sparse_conv_consolidated_maxpool.pt", torch.device(device))
    print("Model:")
    print(model)
    ...
    #model.cnn2_cnn.register_forward_hook(print_layer)
    ...
    global sparse_conv_weights_var, sparse_conv_weights, sparse_conv_biases, sparse_conv_mults_var, sparse_conv_instances, sparse_conv_verify
    sparse_conv_weights_var = torch.tensor([])
    sparse_conv_weights = torch.tensor([])
    sparse_conv_biases = torch.tensor([])
    sparse_conv_mults_var = torch.tensor([])
    sparse_conv_instances = torch.tensor([])
    sparse_conv_verify = torch.tensor([])
    model.cnn2_cnn.register_forward_hook(print_sparse_conv_state)
    ...
    with open(fname, 'x') as f:
        ...
        with open(wname, "wb") as wf:
            sparse_conv_weights.numpy().astype('float32').tofile(wf)
        with open(bname, "wb") as bf:
            sparse_conv_biases.numpy().astype('float32').tofile(bf)
        with open(iname, "wb") as nf:
            sparse_conv_instances.numpy().astype('float32').tofile(nf)
        with open(vname, "wb") as vf:
            sparse_conv_verify.numpy().astype('float32').tofile(vf)
        ...
        ary1 = (sparse_conv_instances.data != 0).int().numpy().sum(axis=1)
        mm = sparse_conv_mults_var.numpy()
        ary8 = mm.min(axis=1)
        ary9 = mm.max(axis=1)
        ary10 = mm.mean(axis=1)
        ary11 = np.median(mm, axis=1)
        ary12 = np.std(mm, axis=1)
        ary4 = sparse_conv_weights_var.numpy().sum(axis=0) # along col (across rows) (column non-sparsity)
        ary5 = sparse_conv_weights_var.numpy().sum(axis=1) # along row (across cols) (row non-sparsity)
