...
def do_profile_test(model, device, batch_size, dataset_name="test", inflation_multiplier=1):
    """
    ...
    for noise in [0.0]:
        ...
        test_loader = torch.utils.data.DataLoader(test_dataset,
                                                  batch_size=batch_size, drop_last=True, num_workers=16)
...
if __name__ == "__main__":
    ...
    fname = fname.replace(':','-')
    if args.batchsize == 0:
        #batch_list = [1,2,4,8,16,32,64,128,256] # CPU example
        #batch_list = [256,512,1024,2048,4096,8192] # GPU example
    ...
    with open(fname, 'x') as f:
        ...
        for trial in range(1,numtrials+1):
            ...
            for batch_size in batch_list:
                ...
                res = do_profile_test(model, device, batch_size, datasetname, inflation_multiplier)
                res = [trial] + res
