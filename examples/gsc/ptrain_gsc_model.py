...
def profile_train(model, loader, optimizer, criterion, device):
    """
    Train the model using given dataset loader.
    Called on every epoch.
    ...
    for data, target in loader:
        ...
        if batch_size ==  data.size(0):
            times[idx] = tim/batch_size
            idx = idx + 1
    times = times[0:idx] # Shorten array to times which are full batch sizes
    avg_time = times.mean()
    med_time = np.median(times) # for some reason times.median() provokes a no attribute 'median' error
    std_time = times.std()
    min_time = times.min()
    max_time = times.max()
...
def profile_training(model, device, batch_size, f):
    """
    Train the model.
    ...
    for epoch in range(EPOCHS):
        ...
        res = profile_train(model=model, loader=train_loader, optimizer=sgd,
              criterion=F.nll_loss, device=device)
        lr_scheduler.step()
        model.apply(rezero_weights)
        model.apply(update_boost_strength)
        ...
        results = test(model=model, loader=valid_loader, criterion=F.nll_loss,
                       device=device)
        #print("Epoch #", epoch, sep="", file=f)
        presults = presults + [results]
        res = [epoch] + res
        print(('{:2d},' + '{:4d},' * 2 + '{:.3f},' * 5).format(res[0], res[1], res[2], res[3], res[4], res[5], res[6],
                                                               res[7]), file=f)
        print("Epoch {}: {}".format(epoch, results))
    epoch = 0
    print('\nEpoch,Accuracy,Loss,Total Correct',file=f)
    for v in presults:
        print('{:2d},{:.6f},{:.6f},{:4d}'.format(epoch, v["accuracy"], v["loss"], v["total_correct"]),file=f)
        epoch = epoch + 1
...
def do_noise_test(model, device):
    ...
    for noise in [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]:
        ...
        results = test(model=model, loader=test_loader, criterion=F.nll_loss,
                       device=device)
        #print("({:.2f},{:.6f},(:.6f},{:4d}").format(noise, results["accuracy"], results["loss"], results["total_count"],file=f)
        print("Noise level: {}, Results: {}".format(noise, results))


def profile_noise_test(model, device, f):
    """
    Test on the noisy data.
    ...
    :type device: torch.device
    """
    print('\nNoise Level,Accuracy,Loss,Total Correct',file=f)
    for noise in [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]:
        noise_wavdata_to_tensor = [LoadAudio(),
                                   FixAudioLength(),
                                   AddNoise(noise),
        ...
        results = test(model=model, loader=test_loader, criterion=F.nll_loss,
                       device=device)
        print(('{:.2f},'+'{:.6f},'+'{:.6f},'+'{:4d}').format(noise, results["accuracy"], results["loss"], results["total_correct"]),file=f)
        print("Noise level: {}, Results: {}".format(noise, results))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batchsize", default=16, type=int, help="Training batch size for epochs after the first (default 16)")
    ...
    model = modelclass(pretrained=False).to(device)
    #print("Model:")
    #print(model)

    fname = ('train_supersparse' if args.supersparse else 'train_sparse') + '_batch' + str(batch_size) + '_' + devstring + '_' + '"' + processor_desc + '"' + '_' + datetime.datetime.now().isoformat(sep='_', timespec='seconds') + '.csv'
    fname = fname.replace(':','-')
    with open(fname, 'x') as f:
        ...
        print('Epoch,Batch Size,Num Batches,Mean,Median,STDev,Min,Max', file=f)
        profile_training(model, device, batch_size, f)
        profile_noise_test(model, device, f)
